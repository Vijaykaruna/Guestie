...
const InvoicePage = ({ guestControl, refreshTick }) => {
  ...
  useEffect(() => { useInvoice.loadGuests?.(); }, [refreshTick]);
    ...
    <div className="admin-page-wrap">
      <div className="admin-hero-card">
        <div>
          ...
          <h1>Invoice</h1>
          <p>Review guest order bills, open food details, and mark pending payments as paid.</p>
        </div>
      </div>
      <div className="admin-info-grid">
        <div className="admin-info-card"><span>Invoices</span><strong>{useInvoice.guestDetailsList.length}</strong><p>Guest billing records.</p></div>
        <div className="admin-info-card"><span>Total Sales</span><strong>₹{totalAmount.toFixed(0)}</strong><p>Amount generated from food orders.</p></div>
        <div className="admin-info-card"><span>Pending Payments</span><strong>{pendingPayments}</strong><p>Need payment confirmation.</p></div>
      </div>
      <div className="admin-content-card">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
          <div className="guest-section-title"><div><span>Payments</span><h2>Invoice Details</h2></div></div>
          {selected.length > 0 && (
        ...
        <p className="text-secondary m-0 mb-2">Click pending to confirm received payment.</p>
        <div className="table-responsive admin-table-wrap">
          <table className="table table-hover align-middle admin-table">
            <thead>
              <tr className="text-center">
                <th><input type="checkbox" onChange={(e) => {
                  if (e.target.checked) setSelected(useInvoice.guestDetailsList.map(g => g._id));
                  else setSelected([]);
                }} checked={selected.length === useInvoice.guestDetailsList.length && useInvoice.guestDetailsList.length > 0} /></th>
                <th>Guest Name</th><th>Room No</th><th>Mobile</th><th>Total Orders</th><th>View Orders</th><th>Total Amount</th><th>Payment</th>
              </tr>
            </thead>
            <tbody>
              {useInvoice.guestDetailsList.length === 0
                ...
                : useInvoice.guestDetailsList.map((guest) => (
                  <tr key={guest._id} className="text-center">
                    <td><input type="checkbox" checked={selected.includes(guest._id)} onChange={() => toggleSelect(guest._id)} /></td>
                    <td className="fw-bold">{guest.name}</td>
                    <td>{guest.roomNumber}</td>
                    <td>{guest.mobile}</td>
                    <td>{guest.orders.length || 0}</td>
                    <td><button className="btn btn-sm btn-primary rounded-2" onClick={() => useInvoice.showOrderedListById(guest._id)}>View</button></td>
                    <td>₹{guest.totalAmount.toFixed(2)}</td>
                    <td>{guest.payment === "Pending"
                      ? <button className="btn btn-sm btn-danger rounded-2" onClick={() => useInvoice.showAlertPaymentStatus(guest._id)}>Pending</button>
