...
const OrderPage = ({ orderControl, refreshTick }) => {
  ...
  const pending = useOrder.ordersList.filter(
    (o) => o.status === "Pending",
  ).length;
  const delivered = useOrder.ordersList.filter(
    (o) => o.status === "Delivered",
  ).length;

  const handleReject = (orderId) => {
    const reason = window.prompt(
      "Why are you rejecting this order? Please enter the reason for guest response:",
    );
  ...
  const handleDeleteSelected = async () => {
    ...
    const success = await orderControl.deleteOrders(selected);
    if (success) {
      setSelected([]);
      useOrder.getAllOrders();
    }
    ...
    <div className="admin-page-wrap">
      <div className="admin-hero-card">
        <div>
          ...
          <h1>Order List</h1>
          <p>
            Track guest food orders in real time, view ordered foods, and update
      ...
      <div className="admin-info-grid">
        <div className="admin-info-card">
          ...
          <strong>{useOrder.ordersList.length}</strong>
          <p>All orders received from guests.</p>
        </div>
        <div className="admin-info-card">
          <span>Pending</span>
          <strong>{pending}</strong>
          <p>Orders waiting for kitchen action.</p>
        </div>
        <div className="admin-info-card">
          <span>Delivered</span>
          <strong>{delivered}</strong>
          <p>Completed and served orders.</p>
        </div>
      </div>
      ...
      <div className="admin-content-card">
        ...
        <p className="text-secondary m-0 mb-2">
          Approve delivered orders or reject unavailable items with reason.
        </p>
              ...
              <tr className="text-center">
                <th>
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked)
                        setSelected(useOrder.ordersList.map((o) => o._id));
                      else setSelected([]);
                    }}
                    checked={
                      selected.length === useOrder.ordersList.length &&
                  ...
                  <tr key={order._id} className="text-center">
                    ...
                    <td>
                      {order.status === "Delivered" ? (
                        <span className="admin-status">Delivered</span>
        ...
        <p className="text-center text-secondary my-2">
          <IoCheckmarkSharp className="text-success" /> Delivered &nbsp;|&nbsp;{" "}
          <FaXmark className="text-danger" /> Rejected with reason
        </p>
      </div>
    </div>
  );
